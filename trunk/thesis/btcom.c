...
int main(void) {
	...
	e_po3030k_init_cam();
	e_po3030k_config_cam((ARRAY_WIDTH -cam_width*cam_zoom)/2,
	  (ARRAY_HEIGHT-cam_heigth*cam_zoom)/2,cam_width*cam_zoom,cam_heigth*cam_zoom,cam_zoom,cam_zoom,cam_mode);
	...
	while(1) {
		...
		if (c<0) { // binary mode (big endian)
			...
			////Elib.Comnands.c_SetCamPar(int width, int height, CamMode mode, Zoom zoom)
			...
			////Elib does not implement it, because school robots do not have this extension.
#ifdef FLOOR_SENSORS
				e_I2Cp_enable();
