...
namespace Elib {
  ...
  /// <para>
  /// Consider following example. We want to accelerate fluently from 0 to 1 speed in 100steps(0,01;0,02;....;0,98;1,00)
  ...
  public partial class Sercom:IDisposable {
    ...
    public Sercom(string portName, int serialPortWriteTimeout, int serialPortReadTimeout) {
      ...
      notSent.NonEmpty += new EventHandler(Send);      
    ...
    public void Write(string command, OkfCallback okf, KofCallback kof,object state, double timeout) {
      if (!disposed) {
        ...
        lock (notSentLock) {
          ...
          p.nSST = notSentSTS.Enqueue(np);
          p.nS = notSent.Enqueue(np);          
          NotAnswered++;                        
        }
        checkNSwhSet();       
      }
    ...
    /// </summary>
    public int NotSent { get { return notSent.Count; } }
    /// <summary>
